...
```
NY NOAA data contains five core variables for all New York state weather station form January 1, 1981 through December 31, 2010. There are `r nrow(weather_df)` rows and `r ncol(weather_df)` columns in this dataset while `r nrow(weather_df)` observations in total. 
...
* The average maximum temperature is `r round(mean(pull(weather_df, tmax), na.rm = TRUE), digits = 2)` tenths of mm with the standard deviation of `r round(sd(pull(weather_df, tmax), na.rm = TRUE), digits = 2)` tenths of mm. The range of between maximum temperature during the period is between `r max(pull(weather_df, tmax), na.rm = TRUE)` tenths of mm and `r min(pull(weather_df, tmax), na.rm = TRUE)` tenths of mm. 

* The average minimum temperature is `r round(mean(pull(weather_df, tmin), na.rm = TRUE), digits = 2)` tenths of mm with the standard deviation of `r round(sd(pull(weather_df, tmin), na.rm = TRUE), digits = 2)` tenths of mm. The range of minimum temperature during the period is between  `r max(pull(weather_df, tmin), na.rm = TRUE)` tenths of mm and `r min(pull(weather_df, tmin), na.rm = TRUE)` tenths of mm. 

* There are `r sum(is.na(weather_df$tmin))` , `r sum(is.na(weather_df$tmax))` and `r sum(is.na(weather_df$snow))` missing values in the variables of minimum and maximum temperature, and snowfall respectively. 
